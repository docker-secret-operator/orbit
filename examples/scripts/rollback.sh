#!/usr/bin/env bash
# rollback.sh — Manual rollback helper for a Docker Rollout-managed service.
#
# Usage:
#   ./examples/scripts/rollback.sh <service>
#
# This script is a thin wrapper around `docker-rollout rollback` that adds:
...
#
# When to use:
#   docker-rollout rollout saves a state file at /tmp/docker-rollout-<service>-state.json
...
control_get /metrics 2>/dev/null \
  | grep 'docker_rollout_connections_failed_total\|docker_rollout_backends_active' || true

# Verify state file exists.
STATE_FILE="/tmp/docker-rollout-${SERVICE}-state.json"
if [[ ! -f "$STATE_FILE" ]]; then
  die "No rollout state found at ${STATE_FILE}. Cannot roll back. Check: docker-rollout status"
fi
...
docker-rollout rollback "${SERVICE}" \
  --control-addr "${CONTROL_ADDR}" \
  ${DOCKER_ROLLOUT_API_TOKEN:+--api-token "${DOCKER_ROLLOUT_API_TOKEN}"} \
  || die "docker-rollout rollback command failed"
...
for i in $(seq 1 "$HEALTH_CHECK_RETRIES"); do
  ACTIVE=$(control_get /metrics 2>/dev/null \
    | grep '^docker_rollout_backends_active ' | awk '{print $2}' || echo 0)
...
die "Service ${SERVICE} has no active backends after rollback. Manual check required: docker-rollout status"
