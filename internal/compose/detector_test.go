...
func TestShouldProxy(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			svc:  compose.Service{Image: "myapp:latest", Ports: []string{"3000:3000"}, XRollout: compose.XRolloutConfig{Skip: true}},
			want: false,
		},
		{
			name: "explicit skip on database",
			svc:  compose.Service{Image: "mysql:8.0", Ports: []string{"3306:3306"}, XRollout: compose.XRolloutConfig{Skip: true}},
			want: false,
		},
		{
