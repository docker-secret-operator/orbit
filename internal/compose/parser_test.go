...
func TestParseBytes_XRolloutSkip(t *testing.T) {
	y := `
version: "3.9"
services:
  app:
...
`
	...
	if !cf.Services["app"].XRollout.Skip {
		t.Error("x-docker-rollout.skip should be true")
	}
}

func TestParseBytes_XRolloutNotInRawFields(t *testing.T) {
	y := `
version: "3.9"
services:
  app:
...
`
	...
	if _, ok := cf.Services["app"].RawFields["x-docker-rollout"]; ok {
		t.Error("x-docker-rollout should NOT appear in RawFields (it must be stripped)")
	}
}
